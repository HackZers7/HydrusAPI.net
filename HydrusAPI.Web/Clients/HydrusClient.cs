...
public class HydrusClient : IHydrusClient
{
    ...
    public HydrusClient(Uri baseAddress, IToken token) : this(
        HydrusClientConfig.CreateDefault(
            baseAddress,
    ...
    /// <param name="baseAddress">Адрес клиента Hydrus.</param>
    /// <param name="token">Ключ доступа.</param>
    public HydrusClient(Uri baseAddress, string token) : this(
        HydrusClientConfig.CreateDefault(baseAddress, token)
    )
    {
    }
